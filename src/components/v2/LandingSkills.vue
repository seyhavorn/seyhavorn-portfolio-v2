          ...
          <div class="space-y-4">
            <div v-for="(skill, si) in cat.skills" :key="si">
              ...
              <div class="w-full h-[6px] bg-white/[0.06] rounded-full overflow-hidden">
                <div
                  class="h-full rounded-full bg-gradient-to-r from-teal-500 to-blue-500 transition-all duration-1000 ease-out skill-bar"
      ...
      <div class="mt-14">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div
            ...
            :key="ci"
            class="glass-card px-6 py-5 flex items-center gap-4 group hover:border-amber-500/30 hover:shadow-[0_0_25px_rgba(245,158,11,0.08)]"
          >
            <div class="w-12 h-12 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform duration-300">
              <!-- Cloud -->
            ...
            <div>
              <h4 class="text-[14px] font-semibold text-white group-hover:text-amber-300 transition-colors duration-300">{{ cert.title }}</h4>
              <p class="text-[12px] text-[var(--text-tertiary)] font-mono">{{ cert.issuer }} · {{ cert.year }}</p>
            </div>
          </div>
        </div>
      </div>
