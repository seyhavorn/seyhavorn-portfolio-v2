<template>
  <section class="mb-8">
    <div class="flex items-center gap-3 font-mono text-[11px] tracking-[4px] uppercase text-teal-400 mb-8 font-semibold pl-2">
    ...
    >
      ...
      <div class="relative z-10 pl-16 mb-6 group">
        <div class="absolute left-[16px] top-1.5 w-[10px] h-[10px] rounded-full bg-teal-400 border-[3px] border-[var(--bg)] shadow-[0_0_12px_rgba(45,212,191,0.8)] group-hover:scale-125 transition-transform duration-300"></div>
        <div class="flex flex-wrap items-center gap-3 mb-1">
          <span class="text-[18px] md:text-[20px] font-semibold text-white tracking-wide">{{ experience.company }}</span>
          <span class="font-mono text-[11px] text-teal-300 bg-teal-900/40 border border-teal-500/30 px-3 py-1 rounded-full font-medium shadow-[0_0_10px_rgba(45,212,191,0.1)]">{{ experience.totalDuration }}</span>
        </div>
      ...
      <div class="space-y-6 relative z-10 pl-12">
        <div 
          ...
          :key="index"
          class="glass-card p-6 md:p-8 relative group"
        >
          <!-- Timeline Connection Dot on Card hover -->
          <div class="absolute -left-[30px] top-8 w-[6px] h-[6px] rounded-full bg-teal-500/50 opacity-0 group-hover:opacity-100 transition-all duration-300 shadow-[0_0_8px_rgba(45,212,191,0.8)]"></div>
          ...
          <div class="text-[13px] text-[var(--text-tertiary)] mb-4 flex items-center gap-2">
            <svg class="w-3.5 h-3.5 fill-current text-blue-400/80" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd"/></svg>
            {{ job.location }}
          </div>
